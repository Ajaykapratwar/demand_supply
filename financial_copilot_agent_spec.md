...
class FinancialChatResponse(BaseModel):
    ...
    model_used: str                    # "haiku" | "sonnet" for cost tracking
```
...
<DrawerShell position="right" width="420px">
  ...
  │     ├── Model badge: "Haiku" | "Sonnet" (for transparency)
...
MODEL_ROUTING = {
    "simple":  "claude-haiku-4-5-20251001",     # ~$0.001/query
    "medium":  "claude-haiku-4-5-20251001",     # ~$0.002/query
    "complex": "claude-sonnet-4-6",             # ~$0.01-0.03/query
}
```
...
|---|---|---|---|
| Simple KPI lookup | "What is our current EVA?" | Haiku | ~$0.001 |
| Trade-off explanation | "What happens if inventory turns increase?" | Haiku + tools | ~$0.002 |
| Scenario reasoning | "Compare air vs ocean freight on margin" | Sonnet | ~$0.010 |
| Strategic recommendation | "Should we defer capacity investment?" | Sonnet + full context | ~$0.030 |
| Executive briefing (pre-computed) | Daily CFO briefing | Sonnet (scheduled) | ~$0.050 (3×/day total) |
...
|---|---|---|---|
| No optimization (all Sonnet, all on-demand) | ~$200/mo | $0 | **~$215/mo** |
| Tiered routing (80% Haiku / 20% Sonnet) | ~$22/mo | $0 | **~$25/mo** |
| + Pre-computed briefings | ~$3/mo saved | $0 | **~$25/mo** |
| + Semantic cache (20–30% hit rate) | ~$5/mo saved | $0–$10 (Redis) | **~$20–28/mo** |
| + Constrained output tokens | ~$3/mo saved | $0 | **~$18–25/mo** |

**Target: <$30/month for 5 users = <$6/user/month**
...
|---|---|---|---|---|
| 1 | "What is our current EVA?" | `get_financial_kpis` | Haiku | Returns EVA value with unit and timestamp |
| 2 | "Show me cash-to-cash trend" | `get_financial_kpis` | Haiku | Returns time-series data, `chart_type="line"` |
| 3 | "What happens if we push inventory turns higher?" | `get_kpi_tradeoffs`, `get_financial_kpis` | Haiku | Lists cascading effects on fill rate, carrying cost |
| 4 | "Compare air vs ocean freight impact on margin" | `run_what_if_scenario`, `get_financial_kpis` | Sonnet | Side-by-side scenario data, `chart_type="bar"` |
| 5 | "Should we defer the capacity investment?" | `run_what_if_scenario`, `get_kpi_tradeoffs`, `get_financial_kpis` | Sonnet | Pros/cons with quantified financial impact |
| 6 | "Why did margin drop this week?" | `get_financial_kpis`, `get_cost_breakdown` | Haiku | Root cause with cost driver breakdown, `chart_type="waterfall"` |
| 7 | "What's the carrying cost of overstock in EMEA?" | `get_financial_kpis`, `get_cost_breakdown` | Haiku | Region-specific carrying cost with % of total |
| 8 | "Give me my daily briefing" | (from cache) | Cache hit | Response in <100ms, `source="cache"` |
| 9 | "What is the trade-off between fill rate and inventory cost?" | `get_kpi_tradeoffs` | Haiku | Quantified trade-off with direction and magnitude |
| 10 | "Run a budget scenario with 10% logistics cost reduction" | `run_what_if_scenario`, `get_financial_kpis` | Sonnet | P&L impact table, `chart_type="waterfall"` |
...
| Model Routing Split | Haiku% vs Sonnet% | Sonnet% >30% → investigate |
...
|---|---|---|---|
| Proactive alerts ("Cash-to-Cash exceeded threshold") | Adoption >50% | ~$0 (reuse alert pipeline + Haiku) | 1 day |
...
| Voice interface for mobile executives | Mobile dashboard mature | ~$0.02/query (Whisper API + Haiku) | 1 week |
...
| Component | Cost |
|---|---|
| LLM API (chat queries, tiered routing) | ~$22/mo |
| LLM API (pre-computed briefings, 3/day) | ~$3/mo |
...
| **Total** | **~$25–35/mo for 5 active users** |
| Per-user cost | **~$5–7/user/month** |
